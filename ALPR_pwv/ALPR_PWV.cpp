...
int main(int argv, char** argc) {
	//findPossiblePlates(filename);

	VideoCapture cap("VID_20180222_131327.mp4");// "VID_20180222_131327.mp4"); // open the default camera
	...
	while (true)
	{
		...
		face_cascade.load("C:/Users/Athinodoros/Desktop/Internship/classifier/cl 2/classifier/cascade.xml");

		// Detect faces
		std::vector<Rect> platesdetected;
		cv::Size size(200, 80);
		face_cascade.detectMultiScale(frame, platesdetected, 1.1, 2, 0 | CASCADE_SCALE_IMAGE, size);
		...
		for (int i = 0; i < platesdetected.size(); i++)
		{
			...
			ellipse(frame, center, Size(platesdetected[i].width*0.5, platesdetected[i].height*0.3), 0, 0, 360, Scalar(255, 0, 255), 4, 8, 0);
		}
		imshow("9 croped and rotated area", frame);
	
		*/
		findPossiblePlatesClassifier(frame, edges);
		if (waitKey(2) == 27)
		{
...
void findPossiblePlatesClassifier(Mat& in, Mat& out) {
	...
	for (int i = 0; i < platesdetected.size(); i++)
	{
		try {
			...
			Point center(platesdetected[i].x + platesdetected[i].width*0.5, platesdetected[i].y + platesdetected[i].height*0.5);
			rectangle(src, platesdetected[i], Scalar(255, 80, 0), 1, LINE_4, 0);
